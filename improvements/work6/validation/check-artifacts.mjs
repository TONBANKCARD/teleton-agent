...
const reportPath = join(root, "AUDIT_V6_REPORT.md");
...
// The 18 confirmed findings filed as issue templates.
const findingIds = Array.from({ length: 18 }, (_, i) => `WORK6-${String(i + 1).padStart(3, "0")}`);
...
if (!existsSync(issuesDir)) {
  ...
  for (const file of files) {
    ...
    const idMatch = frontmatter[1].match(/^finding-id:\s*"?(WORK6-\d{3})"?/m);
...
console.log("work6 audit artifacts are structurally valid");
